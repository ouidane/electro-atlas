...
import { useLogoutMutation } from "@/redux/features/auth-slice";
...
const MyAccount = () => {
  ...
  const { data: userData, isLoading: userLoading } = useGetUserQuery(undefined, { skip: !accessToken });
                ...
                <div className="hidden lg:flex flex-wrap items-center gap-5 py-6 px-4 sm:px-7.5 xl:px-9 border-r xl:border-r-0 xl:border-b border-gray-3">
                  <div className="max-w-[64px] w-full h-16 rounded-full overflow-hidden">
                    <Avatar className="w-16 h-16">
                      <AvatarImage
                        src={userData?.data?.profile?.avatarUrl || undefined}
                        alt={userData?.data?.profile?.givenName || "user"}
                      />
                      <AvatarFallback>
                        {userData?.data?.profile?.givenName?.[0].toUpperCase() || "U"}
                        {userData?.data?.profile?.familyName?.[0].toUpperCase() || ""}
                      </AvatarFallback>
                    </Avatar>
                  </div>
                  ...
                  <div>
                    <p className="font-medium text-dark mb-0.5">
                      {userData?.data?.profile?.fullName || `${userData?.data?.profile?.givenName || "-"} ${userData?.data?.profile?.familyName || ""}`}
                    </p>
                    <p className="text-custom-xs">
                      Member Since {userData?.data?.createdAt ? new Date(userData.data.createdAt).toLocaleString('default', { month: 'short', year: 'numeric' }) : "N/A"}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="flex flex-col gap-4">
                    <p className="flex items-center gap-2.5 text-custom-sm">
                      ...
                      {(() => {
                        ...
                        if (!a) return 'N/A';
                        const parts = [a.line1, a.line2, a.city, a.country, a.postalCode]
                          .filter(Boolean)
                          .map((v) => v || 'N/A');
                ...
                <div className="bg-white shadow-1 rounded-xl p-4 sm:p-8.5">
                  <div className="flex flex-col lg:flex-row gap-5 sm:gap-8 mb-5">
                    <div className="w-full">
                      ...
                      />
                      {errors.givenName && (
                        <span className="text-red text-xs mt-1 block">{errors.givenName.message}</span>
                      )}
                    </div>
                    <div className="w-full">
                      ...
                      />
                      {errors.familyName && (
                        <span className="text-red text-xs mt-1 block">{errors.familyName.message}</span>
                      )}
                    </div>
                  </div>
                  <div className="mb-5">
                  ...
                  <div className="mb-5">
                    ...
                    {errors.address?.line1 && (
                      <span className="text-red text-xs mt-1 block">{errors.address.line1.message}</span>
                    )}
                  </div>
                  <div className="mb-5">
                  ...
                  <div className="flex flex-col lg:flex-row gap-5 sm:gap-8 mb-5">
                    <div className="w-full">
                      ...
                      />
                      {errors.address?.city && (
                        <span className="text-red text-xs mt-1 block">{errors.address.city.message}</span>
                      )}
                    </div>
                    <div className="w-full">
                      ...
                      {errors.address?.country && (
                        <span className="text-red text-xs mt-1 block">{errors.address.country.message}</span>
                      )}
                    </div>
                  </div>
                  <div className="mb-5">
                    ...
                    {errors.address?.postalCode && (
                      <span className="text-red text-xs mt-1 block">{errors.address.postalCode.message}</span>
                    )}
                  </div>
