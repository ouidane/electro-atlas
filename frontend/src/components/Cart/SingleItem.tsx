...
const SingleItem = ({ item }) => {
  ...
  const handleRemoveFromCart = async () => {
    if (token) {
      await removeCartItemApi({ productId: item.product._id });
    } else {
  ...
  const handleIncreaseQuantity = async () => {
    ...
    if (token) {
      await updateCartItemApi({ productId: item.product._id, quantity: newQuantity });
    } else {
      setLocalLoading(true);
      dispatch(updateCartItemQuantity({ ProductId: item.product._id, quantity: newQuantity }));
      setLocalLoading(false);
    }
  };
  ...
  const anyLoading = isRemoving || isUpdating || localLoading;
  return (
    <div
      className={`flex items-center border-t border-gray-3 py-5 px-7.5 ${anyLoading ? 'opacity-50 pointer-events-none' : 'opacity-100 pointer-events-auto'}`}
    >
      <div className="min-w-[400px]">
        <div className="flex items-center justify-between gap-5">
          <div className="w-full flex items-center gap-5.5">
            ...
            <div>
              <h3 className="line-clamp-2 break-words leading-5 text-dark ease-out duration-200 hover:text-blue">
      ...
      <div className="min-w-[180px]">
        <p className="text-dark">${item.product.variant.salePriceDecimal}</p>
      </div>
      ...
      <div className="min-w-[200px]">
        <p className="text-dark">${item.totalPriceDecimal}</p>
      </div>
